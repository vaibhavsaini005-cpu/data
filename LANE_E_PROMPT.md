...
You are running Lane E of a market-intelligence sprint. Repo:
...
3. Research 2-3 REAL, named companies from Lane E's countries: Malaysia,
   Cambodia, Myanmar, Laos, Kazakhstan, Uzbekistan, Georgia, Armenia,
   Azerbaijan. Diverse sectors, mix of failure/pivot/success outcomes.
   Each entry needs at least one real, checkable public source (URL +
   date) — no invented/composite companies.
4. Write one markdown file per entry in lane-e/, following
   ENTRY_TEMPLATE.md.
5. Append the new company names to ledger.json under
   lanes.e.logged_companies.
